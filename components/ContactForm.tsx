...
export default function ContactForm() {
  ...
  return (
    <form onSubmit={submit} className="space-y-4 max-w-2xl">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label className="block text-xs font-bold uppercase tracking-widest text-gray-400 dark:text-white/40 mb-2">Company</label>
          <input type="text" value={form.company} onChange={set('company')} placeholder="Organisation (optional)" className={inputClass} />
        </div>
        <div>
      ...
      <div>
        <label className="block text-xs font-bold uppercase tracking-widest text-gray-400 dark:text-white/40 mb-2">Message *</label>
        <textarea required value={form.message} onChange={set('message')} placeholder="Tell me about your project..." rows={6} className={`${inputClass} resize-none`} />
      </div>
      {status === 'error' && (
